...
In the attached [demo notebook](tinytorchcompile.ipynb), the same expression is run and timedin four different ways:
...
Note: The above times are measured on CPU of Macbook M1 Pro and for a single function call. The actual runtime will be faster on a GPU or if the function is called multiple times due to caching. If you want to see the difference in runtime on a GPU, you can run the [demo notebook](tinytorchcompile.ipynb) on a GPU.
...
jupyter lab tinytorchcompile.ipynb    # the demo notebook
```
...
Attached [demo notebook](tinytorchcompile.ipynb) demonstrates the above pipeline of operator fusion with a simple example and a complex example:
